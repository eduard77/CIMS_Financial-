...
\*Last updated: Sprint 4 F1 complete (2026-05-08). All F1 passing criteria met. ADR-0007 captured Pattern B inbox before code: HMAC-SHA256 over the raw body with a per-spoke shared secret in `Cims:Webhook:Secret`, base64 in `X-Signature`, constant-time compare via `CryptographicOperations.FixedTimeEquals`. New `fin.InboxEvents` table with EventId unique index for idempotency; single-transaction insert + `IPublisher.Publish` + mark-Processed → commit. `POST /api/events/incoming` is allow-anonymous (HMAC is the auth). First subscribed event: `ScheduleActivityCostLoaded_v1` from the Optimisation Engine — the handler resolves FinancialsProject from `CimsProjectId`, finds the open draft revision, and `AddLine`s with the `ActivityId` populated; no-op-and-log when project / budget / draft is missing. Genera BoQ XML 1.0 schema lives in `Financials.Contracts/Schemas/genera-boq-1.0.xsd`; `BoqXmlParser` validates structure + version + decimals with invariant culture; `ImportBoqCommand` auto-opens a draft revision (using the document's RevisionReason) if none is open and bulk-adds lines via the existing aggregate path. `/projects/{id}/budget` gains an Import-BoQ section (5 MB upload cap) gated by BudgetWrite. 113 unit + 12 Infrastructure ring + 13 in-process slice integration. Outbox half of Pattern B intentionally deferred to F3 (Sprint 7-9 — when Financials first publishes change events). Same browser-demo deferral as previous sprints. Update this footer every sprint with what changed.\*
