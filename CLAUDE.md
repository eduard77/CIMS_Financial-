...
\*Last updated: Sprint 2 F0 complete (2026-05-08). All F0 passing criteria met under ADR-0005 (CIMS catalogs + Financials commercial overlay). Four new Pattern A endpoints (contract templates, tax regime, cost codes, role assignments) added to `ICimsClient` with the same Polly + 60s cache + bearer + correlation chain. New `ProjectCommercialConfiguration` aggregate (1:1 with `FinancialsProject`) holds Financials-owned `RetentionScheme` + `PaymentTerms` value objects + contract template selection. `/projects/{id}/setup` page surfaces the four sections with independent loading/error states; CBS depth ≥ 4 banner satisfies F0 item 1. Two new authorization policies (`SetupRead`, `SetupConfigure`) plus `FinancialsRolePermissions` map for the audit/legal trail. 70 unit tests + 8 Infrastructure ring + 6 in-process slice integration. Manual browser demo and CIMS-staging E2E still deferred (same reason: no dev CIMS / OIDC running). Update this footer every sprint with what changed.\*
