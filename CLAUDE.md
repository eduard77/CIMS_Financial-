...
\*Last updated: Sprint 5 F2 foundation complete (2026-05-08). F2 #1 met. ADR-0008 captured the Commitment aggregate before code: single root with `CommitmentType` discriminator (Subcontract | PurchaseOrder), Status state machine (Draft → Active → Closed), owned `CommitmentLine` children mirroring BudgetLine. New `ICimsClient.GetOrganisationAsync` + `ListOrganisationsAsync` (Pattern A, 60s cache) for counterparty resolution. Aggregate methods enforce: AddLine only on Draft + currency-match; OverrideRetention only on Subcontract+Draft; Activate requires lines + userId; Close only from Active; no double-Activate / double-Close. Migration `AddCommitments` adds two tables (Commitments + CommitmentLines) with cascade FKs and unique index `(FinancialsProjectId, Type, Reference)` + supplementary CounterpartyCimsOrganisationId index. Application slice: Raise / AddCommitmentLine / Activate / Close commands + GetCommitmentsForProject query (resolves counterparty name via Pattern A with cache amortisation). `/projects/{id}/commitments` page lists commitments + raise form + per-row Activate/Close. Two new auth policies (CommitmentsRead / CommitmentsWrite); FinancialsRolePermissions map updated. 115 unit + 12 Infrastructure ring + 16 in-process slice integration. F2 #2 over-commitment guard, #3 bonds/warranties, and #4 reconciliation dashboard all land in Sprint 6 alongside the inline line-add UI. Same browser-demo deferral as previous sprints. Update this footer every sprint with what changed.\*
