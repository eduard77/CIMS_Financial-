...
\*Last updated: Sprint 3 F1 foundation complete (2026-05-08). F1 items 3 + 4 met (audit trail on revision approval; multi-level rollup reconciles to the penny). ADR-0006 captured the Budget aggregate shape: Budget root 1:1 with FinancialsProject, owned BudgetRevision children with monotonic RevisionNumber, owned BudgetLine grandchildren with snapshot description + CimsCostCodeId reference + Money UnitRate/Amount. `Money` value object lives in Domain.Common (decimal + ISO 4217 currency, Add/Subtract/Multiply with currency-mismatch guards, GBP default per CLAUDE.md §2 #7). Migration `AddBudgets` adds three tables (Budgets, BudgetRevisions, BudgetLines) with cascade FKs, unique indexes (FinancialsProjectId; BudgetId+RevisionNumber; BudgetRevisionId+LineNumber). Application slice: CreateBudget / OpenBudgetRevision / AddBudgetLine / ApproveBudgetRevision commands + GetBudget / GetBudgetRollup queries; approver pulled from ICurrentUserService, no caller-supplied id. UI `/projects/{id}/budget` lets QS create budget, open revisions, add lines, approve; en-GB Money formatting; rollup by cost code and by work package. Three new auth policies (BudgetRead/Write/Approve); FinancialsRolePermissions map updated. F1 imports (NRM2-style XML, MS Project/P6 via Pattern B subscription) deferred to Sprint 4. 104 unit tests + 11 Infrastructure ring + 9 in-process slice integration. Same browser-demo deferral as Sprint 1+2. Update this footer every sprint with what changed.\*
