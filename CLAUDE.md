...
\*Last updated: Sprint 1 vertical slice complete (2026-05-08). F0 item 5 ("Zero duplicate data entry between CIMS and Financials") satisfied for projects: a logged-in user picks a CIMS project, confirms it for Financials, and sees the confirmed list. Real Pattern A `CimsClient` (typed `HttpClient` + Polly + 60s `IMemoryCache` + bearer-forwarding + correlation-id) per ADR-0002; Sprint 0 stub deleted. CIMS-issued JWT validated locally via OIDC discovery against `https://auth.genera-systems.com` audience `financials` per ADR-0003. `IAuditable` + `AuditingSaveChangesInterceptor` per ADR-0004 — null `UserId` on audited save throws. `FinancialsProject` aggregate + `AddFinancialsProjects` migration + repository pattern. MediatR pipeline behaviours (validation, logging) + hand-rolled `Result<T>`. MudBlazor pages at `/projects` and `/projects/confirm` gated by `Authorize(Policy=...)`. 45 unit tests + 5 infrastructure ring + 3 in-process slice integration. Manual browser demo and CIMS-staging E2E deferred (no dev CIMS / dev OIDC running locally). Update this footer every sprint with what changed.\*
