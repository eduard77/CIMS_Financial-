...
\*Last updated: Sprint 7 F3 NEC4 skeleton complete (2026-05-13). F3 #1 met for NEC4; JCT (#2), bidirectional CIMS RFI link (#3), schedule + budget impact publication (#4 + #5), and BSA golden-thread documentation (#6) all land in Sprints 8 + 9. ADR-0011 accepted before code with two CLAUDE.md §13 user decisions: NEC4 SLAs as per-project policy with conservative defaults (not hard-coded clause text); clock expiry is read-side only — never blocks transitions, because NEC4 §61.3 treats late quotation as a potential deemed acceptance, not a rejection. `ChangeEvent` aggregate root, single-root with `ChangeEventType ∈ EarlyWarning | CompensationEvent` discriminator, flat `ChangeEventStatus` enum, transition methods re-assert `Type`. Lifecycles: EW Notified → Reduced/Closed → Closed; CE Notified → Quoted → Assessed → Implemented (Rejected from any pre-Implemented state). Schema hooks for Sprints 8 + 9: nullable `SourceCimsRfiId` (RFI back-link), nullable `EstimatedNetEffect` Money (budget impact). `Nec4SlaPolicy` value object on `ProjectCommercialConfiguration` (1–365 calendar days, defaults 7/21/14/7); editable on the Setup page. Read-side `ChangeEventClockProjection` returns the active stage + remaining days + breach flag per change event; UI renders Error/Warning/Info chip. Migration `AddChangeEventsAndNec4SlaPolicy` adds four SLA columns (existing rows backfill) + new `fin.ChangeEvents` table with unique `(FinancialsProjectId, Type, Reference)` index. Seven commands (Raise / SubmitQuotation / Assess / Implement / Reject / ReduceEarlyWarning / CloseEarlyWarning) + `ListChangeEventsForProjectQuery` with clock projection. New `/projects/{id}/change-events` page with raise form, transition buttons, MudDialogs for quote amount and rejection reason. Two new auth policies (`ChangeEventsRead` / `ChangeEventsWrite`) wired through FinancialsRolePermissions + Program.cs. Tests: domain `ChangeEventTests` (lifecycle coverage + type-mismatch refusal + currency-match on quote + idempotent RFI link) + `Nec4SlaPolicyTests` (defaults, boundary, out-of-range); application `ChangeEventClockProjectionTests` (Contractor/PM/EW stages + breach + terminal-null); slice `ChangeEventSliceTests` (full CE + EW + reject + duplicate-reference rejection). Sprint 8 picks up JCT + CIMS RFI link + EWR-to-CE promotion. Sprint 9 picks up Pattern B publish to Optimisation and F1 budget-impact wire-through. Update this footer every sprint with what changed.\*

\*Sprint 6 F2 complete (2026-05-13). F2 #2 + #3 + #4 met — F2 fully closed. ADR-0009 accepted before code: over-commitment guard as `OverCommitmentPolicy` value object on `ProjectCommercialConfiguration` (Mode ∈ Disabled / Warn / HardBlock + Money tolerance). Default Warn / 0 GBP (Sprint 6 user decision). Enforcement at Activate only; `IOverCommitmentEvaluator` aggregates `committed` vs `budget + tolerance` per CIMS cost code; pre-flight `EvaluateCommitmentImpactQuery` powers the UI banner. ActivateCommitmentCommand branches: HardBlock + breaches → Result.Failure with per-cost-code detail; Warn + breaches → success + structured Serilog warning; Disabled → no read. F3 hook left for approved-changes injection in Sprints 7–9. ADR-0010 accepted: separate `CommitmentSecurity` aggregate keyed by CommitmentId, single table with SecurityType ∈ Bond / Warranty / Insurance discriminator. Lifecycle Active → Superseded (renewal pointer) / Cancelled; Expired is a read-side projection. `CommitmentSecurityAlertWindow` thresholds at 30/14/7 days. Add + list + cancel commands ship (renewal UI deferred — `SupersedeBy` exists in the aggregate); duplicate `(CommitmentId, Type, Reference)` rejected; can't add to a Closed commitment. Migration `AddOverCommitmentPolicyAndCommitmentSecurities` adds three columns to ProjectCommercialConfigurations (existing rows backfill to Warn / 0 / 'GBP') + a new CommitmentSecurities table with cascade FK + unique index + Issuer/SupersededBy indices. `GetBudgetReconciliationQuery` returns per-CIMS-cost-code rows + totals + `InvariantHolds` (`committed + uncommitted = budget + approved changes` within £0.01). New `/projects/{id}/reconciliation` page with invariant banner and per-cost-code grid. Commitments page gains per-row guard chip + securities panel with 30/14/7 colour-coded alert chips. Setup page gains policy editor. Three new auth policies (CommitmentsSecuritiesRead / Write, ReconciliationRead) wired into FinancialsRolePermissions + Program.cs policy loop. Test count grew ~50: 14 new domain, ~25 new application (evaluator boundary tests, alert-window boundaries 0/1/7/8/14/15/30/31, activate handler under three modes, reconciliation handler proves invariant holds even when intentionally over-committed), and three new Testcontainers slice suites (OverCommitmentGuardSliceTests / CommitmentSecuritiesSliceTests / ReconciliationDashboardSliceTests). Same browser-demo and CIMS-staging-E2E deferrals as previous sprints. Per-cost-code tolerance and expiry-notification dispatch are future ADRs. Sprints 7–9 (F3 Change management) are next — NEC4 + JCT lifecycles, RFI bidirectional links, schedule + budget impact, statutory clocks. Update this footer every sprint with what changed.\*
