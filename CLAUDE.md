...
\*Last updated: Sprint 6 F2 complete (2026-05-08). All F2 passing criteria met. ADR-0009 captured the three Sprint 6 calls: OverCommitmentGuard VO on `ProjectCommercialConfiguration` (Mode: Warn|HardBlock, default Warn); single `CommitmentInsurance` aggregate with `InsuranceCategory` (Bond|Warranty|Insurance) + `InsuranceSubTypes` catalogue (PerformanceBond, AdvancePaymentBond, RetentionBond, PublicLiability, ProfessionalIndemnity, EmployersLiability, ContractorsAllRisks, Workmanship, ProductWarranty); expiry alerts dashboard-only (no background email until a notification ADR ships). Migrations `AddOverCommitmentGuard` + `AddCommitmentInsurances`. Activate now loads PCC + budget + active commitments to compute over-commitment per cost code; HardBlock returns Result.Failure with breach detail; Warn returns Result<ActivateCommitmentResult>.Success with warnings list. Insurance commands: Register + Cancel; queries: GetInsuranceExpiriesForProjectQuery (Critical<7 / Warning<14 / Info<30 + ordered by days remaining), GetCommitmentReconciliationQuery (per-cost-code Budget − Committed = Uncommitted with IsOverCommitted flag). `/projects/{id}/commitments` page gains reconciliation panel + insurance-expiry panel with colour coding. F3 (Change management — NEC4 + JCT lifecycles, statutory clocks, schedule impact via Pattern B) is Sprints 7-9. Same browser-demo deferral. Update this footer every sprint with what changed.\*
